...
final class CodexTransportFactory(
    sessionOpener: McpConnectionConfig => Either[McpError.ConnectionUnavailable.type, CodexTransportFactory.CodexSessionHandle]
):
  import CodexTransportFactory.CodexSessionHandle

  def this() = this(CodexTransportFactory.openWithSdk)

  def create(config: McpConnectionConfig): Either[McpError.ConnectionUnavailable.type, CodexSessionHandle] =
    sessionOpener(config)

object CodexTransportFactory:
  trait CodexSessionHandle:
  ...
  private final case class SdkCodexSessionHandle(client: McpSyncClient) extends CodexSessionHandle:
  ...
  ): Either[McpError.ConnectionUnavailable.type, CodexSessionHandle] =
    toConnectionUnavailable {
      ...
      case Right(client) =>
        toConnectionUnavailable(client.initialize()) match
          case Right(_) => Right(SdkCodexSessionHandle(client))
          case Left(error) =>
